    ...
    public class CriminalService : ApiController
    {
        ...
        public static bool addCommitedRelationship(CommitedRelationship newR)
        {
            ...
            try
            {
                ...
                queryDict.Add("crimeName", newR.crimeName);
                queryDict.Add("caution", newR.caution);

                var query = new Neo4jClient.Cypher.CypherQuery("Match(criminal:Criminal) Match(crime:Crime) Where criminal.Id={criminalId} and crime.name={crimeName} CREATE UNIQUE (crime)<-[c:COMMITED{caution:\""+newR.caution+"\"}]-(criminal)",
                                                                queryDict, CypherResultMode.Projection);
